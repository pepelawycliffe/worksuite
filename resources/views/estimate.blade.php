                                                    ...
                                                    @foreach($estimate->items as $item)
                                                        @if($item->type == 'item')
                                                            <tr>
                                                                ...
                                                                <td class="text-right"> {{ currency_formatter($item->unit_price,$estimate->currency->currency_symbol) }} </td>
                                                                <td class="text-right">{{ currency_formatter($item->amount,$estimate->currency->currency_symbol) }} </td>
                                                            </tr>
                                                        @endif
                                                    @endforeach
                                                    </tbody>
                                        ...
                                        <div class="col-xs-12">
                                            <div class="pull-right m-t-30 text-right">
                                                <p>@lang("modules.invoices.subTotal")
                                                    :{{ currency_formatter($estimate->sub_total,$estimate->currency->currency_symbol) }}</p>

                                                @if ($discount > 0)
                                                    <p>@lang("modules.invoices.discount")
                                                    :{{ currency_formatter($discount ,$estimate->currency->currency_symbol)}} </p>

                                                @endif
                                               
                                                @foreach($taxes as $key=>$tax)
                                                    <p>{{ strtoupper($key) }}
                                                        :{{ currency_formatter($tax,$estimate->currency->currency_symbol) }} </p>
                                                @endforeach
                                                <hr>
                                                <h3><b>@lang("modules.invoices.total")
                                                        :</b>{{ currency_formatter($estimate->total,$estimate->currency->currency_symbol) }}
                                                </h3>
                                            </div>
